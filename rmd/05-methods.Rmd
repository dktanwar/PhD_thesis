...
(ref:mfc1) \textbf{Overview of version control system (VCS).} Adapted from [@chacon2021].
...
GitHub ([github.com](https://github.com)) a distributed
...
`FastQC` [@andrews2012]. `FastQC` provides a comprehensive view of the data,
...
(ref:mfc3) \textbf{Workflow for quality check and quality control for sequencing data.} QC: Quality Control.
...
used to generate an overview report with `multiQC` [@ewels2016], which is
...
[@patro2017] and `Kallisto` [@bray2016] instead perform pseudo-alignment.
...
Repeat Masker as in [@gapp2020]. Downstream analyses including
differential analysis; differential transcript usage and
differential isoform switching; and functional analyses, are performed on the pseudo-alignment from `Salmon`.
...
data using `sambamba` [@tarasov2015], and further, the quality issues with
PCR duplicates are analyzed using `dupRadar` [@sayols2016]. `dupRadar` aids
...
`plgINS`, a versatile `R` package, ([github.com/ETHZ-INS/plgINS](https://github.com/ETHZ-INS/plgINS)),
...
multiGSEA ([github.com/lianos/multiGSEA](https://github.com/lianos/multiGSEA)), which facilitates the
analysis with GOseq [@young2010], fGSEA [@korotkevich2016], Correlation
Adjusted MEan RAnk gene set test (CAMERA) and camera pre-ranked
(CAMERA-PR) [@wu2012], ROtation testing using MEan Ranks (ROMER) and fry
[@ritchie2015], and ROtAtion gene Set Testing (ROAST) [@wu2010]. An
advantage of using multiGSEA is that it has standardized input and
output and provides a Shiny app for exploratory data analysis ([github.com/lianos/multiGSEA.shiny](https://github.com/lianos/multiGSEA.shiny)). With multiGSEA, it is
easy to specify to use testing relative to a threshold (TREAT) method
[@mccarthy2009], which takes a user-specified log fold change cut-off
...
(ref:mfc5) \textbf{Workflow for ATAC sequencing data analysis.} QC: Quality Control, NFF: Nucleosome Free Fragments.
...
order for the read `STAR`t site to reflect the centre of where Tn5 binds
...
data (BAM files) are adjusted for the read `STAR`t sites to represent the
...
csaw package [@lun2015]. Peak regions that did not have at least 15
...
normalization method [@robinson2010] and differential accessibility
...
with the rGREAT package ([github.com/jokergoo/rGREAT](https://github.com/jokergoo/rGREAT)), which is
a wrapper around the Genomic Regions Enrichment of Annotations Tool
(GREAT) [@mclean2010]. Transcription factor motif enrichment analysis is
performed using the marge package [@amezquita2018], which is a wrapper
around the Homer tool [@heinz2010].
...
from [http://labshare.cshl.edu/shares/mhammelllab/www-data/TEtranscripts/TE_GTF/mm10_rmsk_TE.gtf.gz](http://labshare.cshl.edu/shares/mhammelllab/www-data/TEtranscripts/TE_GTF/mm10_rmsk_TE.gtf.gz%20on%2003.02.2020). The GTF file provides hierarchical information about TEs: **Class**
...
package `Rsubread` [@liao2019] and were summarized to Subtypes (level 3),
...
(ref:mfc6) \textbf{Workflow for ChIP sequencing data analysis.} QC: Quality Control.
...
are removed from the aligned data, as suggested in [@royo2016], and
...
(ref:mfc7) \textbf{Workflow for Bisulfite sequencing data analysis.} QC: Quality Control, DML: differentially methylated loci, DMR: differentially methylated regions, CNV: copy-number variation.
...
the bisulfite sequencing data is performed using `Bismark` [@krueger2011]
on a genome index built using `Bismark`_genome_preparation. Methylation
information for individual cytosines (methylation calls) is extracted
using the `Bismark`_methylation_extractor tool from the `Bismark` package.
...
[@thurman2012; @harrow2012]; the proximal regions are further divided
...
(**step 1**). Next, these three groups are separated based on whether
the biological groups are more or less accessible (**step 2**). This
...
based on their up/ down expression (**step 3**). Moreover, we
...
genes [@fu2019]; H3K27ac, which is associated with the higher activation
of transcription and therefore defined as an active enhancer mark
[@zhang2020]; or dual H3K4me3/H3K27ac enrichment [@beacon2021]; as well
as H3K27me3 enrichment, which is involved in transcriptional silencing
[@bogliotti2012] (**step 4**). The schematic is shown in Figure \ref{fig:mf8}. Furthermore, gene ontology and
...
low). Please see [Chapter 2](#chapter2) for an application of this multi-omics
analysis.
...
**ATAC-seq:** ATAC-seq data is the `STAR`ting point and we first find the
...
parts(mean_mode = "w0") in a window of 50bp.


v\_{w 0}=\\frac{v\_{w} W}{W+W\^{\\prime}}

{W} is the sum of width of the intersected parts \\left(\\sum\_{i}\^{n}
w\_{i}\\right) and W\^{\\prime} is the sum of width for the
...
**ChIP-seq:** ChIP-seq data coverage for 1kb upstream and downstream from
...
mean between the intersected parts and un-intersected parts (mean_mode =
"w0") in a window of 50bp.


**BS-seq:** BS-seq data methylation values for 1kb upstream and downstream
...
mean of all the signals (mean_mode = "absolute") in a window of 50bp.

v\_{a}=\\frac{\\sum\_{i}\^{n} x\_{i}}{n}
